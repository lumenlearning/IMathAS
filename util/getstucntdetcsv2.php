<?php
	if (isset($_GET['auth'])) {
		...
		if (empty($CFG['statsauth']) || $CFG['statsauth'] != $_GET['auth']) {
			exit;
		}
	} else {
		require("../init.php");
	...
	//
	$query = "SELECT g.lumen_guid AS group_lumen_guid,g.name AS gname,u.id AS userid, u.LastName,u.FirstName,c.id,c.name AS cname,COUNT(DISTINCT s.id) AS cnt,u.email,g.parent,g.grouptype,c.ancestors,c.level FROM imas_students AS s JOIN imas_teachers AS t ";
	// #### End OHM-specific code #######################################################
	// #### End OHM-specific code #######################################################
	// #### End OHM-specific code #######################################################
	// #### End OHM-specific code #######################################################
	// #### End OHM-specific code #######################################################
	$query .= "ON s.courseid=t.courseid AND s.created_at>$start ";
	if ($end != $now) {
		$query .= "AND s.created_at<$end ";
	}
	...
	while ($row = $stm->fetch(PDO::FETCH_ASSOC)) {
		...
		if (!in_array($row['id'],$seencid)) {
			...
			$userdata[] = array($row['cname'],$row['id'],$row['cnt'],$islti,$sourcetemplate,$row['level']);
		} else {
			$userdata[] = array($row['cname'],$row['id'],$row['cnt'].'(*)',$islti,'',$row['level']);
			//$userdata .= "<sup>*</sup>";
		}
		//$userdata .= "</li>";
	}

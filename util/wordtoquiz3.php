...
if (!isset($_POST['text'])) {
	...
	echo '<p>Copy-paste in the quiz below.  To be processed correctly:</p><ul><li>Auto-number the questions</li><li>Auto-number the answers as a sub-list</li><li>Mark the correct answer with a * at the end</li></ul>';
	echo '<p>Title: <input type="text" size="40" name="title"/></p>';
	$ex = htmlentities('<ol>
<li>An example question<ol>
...
foreach ($pts as $c=>$pt) {
	...
	$prompt = preg_replace('/^\d+(\.|\))\s+/','',trim($prompt));

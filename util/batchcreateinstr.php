...
if (isset($_POST['groupid']) && is_uploaded_file($_FILES['uploadedfile']['tmp_name'])) {
  ...
  while (($data = fgetcsv($handle,2096))!==false) {
    ...
    while (isset($data[$i]) && $data[$i]!='' && intval($data[$i])>0) {
      ...
      $query = "INSERT INTO imas_courses (name,ownerid,enrollkey,allowunenroll,copyrights,msgset,toolset,UIver,showlatepass,itemorder,available,istemplate,deftime,deflatepass,theme,ltisecret,blockcnt,created_at) ";
      $query .= "SELECT name,:ownerid,enrollkey,allowunenroll,copyrights,msgset,toolset,UIver,showlatepass,:itemorder,available,0,deftime,deflatepass,theme,'',1,created_at ";

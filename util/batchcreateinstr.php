...
if (isset($CFG['hooks']['util/batchcreateinstr'])) {
    $prepend = '/' == substr($CFG['hooks']['util/batchcreateinstr'], 0, 1) ? '' : __DIR__ . '/../';
    require($prepend . $CFG['hooks']['util/batchcreateinstr']);
}
...
if (isset($_POST['groupid']) && is_uploaded_file($_FILES['uploadedfile']['tmp_name'])) {
  ...
  while (($data = fgetcsv($handle,2096))!==false) {
    ...
    $stm->execute(array(':SID'=>Sanitize::stripHtmlTags($data[0]), ':password'=>$hashpw, ':FirstName'=>Sanitize::stripHtmlTags($data[2]), ':LastName'=>Sanitize::stripHtmlTags($data[3]),
            ':rights'=>40, ':email'=>Sanitize::emailAddress($data[4]), ':groupid'=>$newusergroupid, ':homelayout'=>$homelayout, ':created_at'=>time()));
    ...
    while (isset($data[$i]) && $data[$i]!='' && intval($data[$i])>0) {
      ...
      $query = "INSERT INTO imas_courses (name,ownerid,enrollkey,hideicons,picicons,allowunenroll,copyrights,msgset,toolset,UIver,showlatepass,itemorder,available,istemplate,deftime,deflatepass,theme,ltisecret,blockcnt,created_at) ";
      $query .= "SELECT name,:ownerid,enrollkey,hideicons,picicons,allowunenroll,copyrights,msgset,toolset,UIver,showlatepass,:itemorder,available,0,deftime,deflatepass,theme,'',1,created_at ";

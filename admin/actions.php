...
switch($_POST['action']) {
	...
	case "addcourse":
		...
		if ($_POST['action']=='modify') {
			...
			#### Begin OHM-specific code #####################################################################
			#### Begin OHM-specific code #####################################################################
			$destUIver = isset($_POST['assess-version']) && 'Old Version' == $_POST['assess-version'] ? 1 : 2;
			...
			$query = "INSERT INTO imas_courses (name,ownerid,enrollkey,hideicons,picicons,allowunenroll,copyrights,msgset,toolset,showlatepass,itemorder,available,startdate,enddate,istemplate,deftime,deflatepass,latepasshrs,theme,level,ltisecret,dates_by_lti,blockcnt,UIver,created_at) VALUES ";
			$query .= "(:name, :ownerid, :enrollkey, :hideicons, :picicons, :allowunenroll, :copyrights, :msgset, :toolset, :showlatepass, :itemorder, :available, :startdate, :enddate, :istemplate, :deftime, :deflatepass, :latepasshrs, :theme, :level, :ltisecret, :ltidates, :blockcnt, :UIver, :created_at);";
			$stm = $DBH->prepare($query);

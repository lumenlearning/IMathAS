...
<?php
	...
	$query = "SELECT imas_fakeusers.id,imas_fakeusers.LastName,imas_fakeusers.FirstName FROM imas_fakeusers,imas_students WHERE ";
	$query .= "imas_students.userid=imas_fakeusers.id AND imas_students.courseid='$cid'";
	$result = mysql_query($query) or die("Query failed : $query " . mysql_error());
	$stulist = array();
	...
	$query = "SELECT imas_fakeusers.id,imas_fakeusers.LastName,imas_fakeusers.FirstName FROM imas_fakeusers,imas_teachers WHERE ";
	$query .= "imas_teachers.userid=imas_fakeusers.id AND imas_teachers.courseid='$cid'";
	$result = mysql_query($query) or die("Query failed : $query " . mysql_error());
	while ($row = mysql_fetch_row($result)) {
		$stulist[$row[0]] = "{$row[1]}, {$row[2]}";
	}

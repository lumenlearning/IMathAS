...
<?php
	...
	$query .= "JOIN imas_msgs ON imas_msgs.msgto=imas_fakeusers.id WHERE imas_msgs.msgfrom='$userid'";
	if ($filtercid>0) {
...
?>
	...
	<tbody>
<?php
	$query = "SELECT imas_msgs.id,imas_msgs.title,imas_msgs.senddate,imas_fakeusers.LastName,imas_fakeusers.FirstName,imas_msgs.isread FROM imas_msgs,imas_fakeusers ";
	$query .= "WHERE imas_fakeusers.id=imas_msgs.msgto AND imas_msgs.msgfrom='$userid' AND (imas_msgs.isread&4)=0 ";
	if ($filtercid>0) {
		$query .= "AND imas_msgs.courseid='$filtercid' ";
	}
	if ($filteruid>0) {

	...
	*/
	...
	if (isset($_GET['add'])) {
		if (isset($_POST['subject'])) {
			...
			$query = "SELECT msgnotify,email FROM imas_fakeusers WHERE id='{$_POST['to']}'";
			$result = mysql_query($query) or die("Query failed : $query " . mysql_error());
			...
			if (isset($_GET['to'])) {
				$query = "SELECT iu.LastName,iu.FirstName,iu.email,i_s.lastaccess,iu.hasuserimg FROM imas_fakeusers AS iu ";
				...
				if ($isteacher || $msgset<2) {
					$query = "SELECT imas_fakeusers.id,imas_fakeusers.FirstName,imas_fakeusers.LastName FROM ";
					$query .= "imas_fakeusers,imas_teachers WHERE imas_fakeusers.id=imas_teachers.userid AND ";
					$query .= "imas_teachers.courseid='$cid' ORDER BY imas_fakeusers.LastName";
					$result = mysql_query($query) or die("Query failed : $query " . mysql_error());
					while ($row = mysql_fetch_row($result)) {
						echo "<option value=\"{$row[0]}\">{$row[2]}, {$row[1]}</option>";
					}
					$query = "SELECT imas_fakeusers.id,imas_fakeusers.FirstName,imas_fakeusers.LastName FROM ";
					$query .= "imas_fakeusers,imas_tutors WHERE imas_fakeusers.id=imas_tutors.userid AND ";
					...
					$query .= "ORDER BY imas_fakeusers.LastName";
					$result = mysql_query($query) or die("Query failed : $query " . mysql_error());
					while ($row = mysql_fetch_row($result)) {
						echo "<option value=\"{$row[0]}\">{$row[2]}, {$row[1]}</option>";
					} 
				...
				if ($isteacher || $msgset==0 || $msgset==2) {
					$query = "SELECT imas_fakeusers.id,imas_fakeusers.FirstName,imas_fakeusers.LastName FROM ";
					$query .= "imas_fakeusers,imas_students WHERE imas_fakeusers.id=imas_students.userid AND ";
					$query .= "imas_students.courseid='$cid' ORDER BY imas_fakeusers.LastName";
					$result = mysql_query($query) or die("Query failed : $query " . mysql_error());
					while ($row = mysql_fetch_row($result)) {
						echo "<option value=\"{$row[0]}\">{$row[2]}, {$row[1]}</option>";
					}
...
<?php
	...
	echo '>All</option>';
	$query = "SELECT DISTINCT imas_fakeusers.id, imas_fakeusers.LastName, imas_fakeusers.FirstName FROM imas_fakeusers ";
	$query .= "JOIN imas_msgs ON imas_msgs.msgfrom=imas_fakeusers.id WHERE imas_msgs.msgto='$userid'";
	if ($filtercid>0) {
...
?>
	...
	<tbody>
<?php
	$query = "SELECT imas_msgs.id,imas_msgs.title,imas_msgs.senddate,imas_msgs.replied,imas_fakeusers.LastName,imas_fakeusers.FirstName,imas_msgs.isread,imas_courses.name,imas_msgs.msgfrom,imas_fakeusers.hasuserimg ";
	$query .= "FROM imas_msgs LEFT JOIN imas_fakeusers ON imas_fakeusers.id=imas_msgs.msgfrom LEFT JOIN imas_courses ON imas_courses.id=imas_msgs.courseid WHERE ";

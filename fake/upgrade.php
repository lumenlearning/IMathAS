...
if (!empty($dbsetup)) {  //initial setup - just write upgradecounter.txt
	...
	if ($last==$latest) {
		...
		if ($last < 7) {
			...
			$query = "SELECT imas_students.id,imas_fakeusers.email FROM imas_students JOIN imas_fakeusers ON imas_fakeusers.id=imas_students.userid AND imas_fakeusers.SID LIKE '%~%~%'";
			$result = mysql_query($query) or die("Query failed : " . mysql_error());
		...
		if ($last < 8) {
			...
			$query = "SELECT u.id,t.id,t.courseid FROM imas_fakeusers as u JOIN imas_teachers as t ON u.id=t.userid AND u.rights=15";
			$result = mysql_query($query) or die("Query failed : " . mysql_error());
			$lastuser = -1;
			while ($row = mysql_fetch_row($result)) {
				if ($row[0]!=$lastuser) {
					$query = "UPDATE imas_fakeusers SET rights=10 WHERE id='{$row[0]}'";
					mysql_query($query) or die("Query failed : " . mysql_error());
					$lastuser = $row[0];
				}
		...
		if ($last < 9) {
			...
			if (isset($_POST['diag'])) {
				...
				if (mysql_num_rows($result)>0) {
					...
					$users = array();
					foreach ($ow as $ogrp) {
						$query = "SELECT id,LastName,FirstName FROM imas_fakeusers WHERE groupid='$ogrp' AND rights>59 ORDER BY id";
						$result = mysql_query($query) or die("Query failed : " . mysql_error());
						if (mysql_num_rows($result)==0) {
		...
		if ($last < 26) {
			 $query = "ALTER TABLE `imas_fakeusers` ADD `homelayout` VARCHAR(32) NOT NULL DEFAULT '|0,1,2||0,1'"; 
			 $res = mysql_query($query);
			 if ($res===false) {
				 echo "<p>Query failed: ($query) : ".mysql_error()."</p>";
			 }
		...
		if ($last<29) {
			...
			$query = 'ALTER TABLE `imas_fakeusers` CHANGE `homelayout` `homelayout` VARCHAR( 32 ) NOT NULL DEFAULT \'|0,1,2||0,1\'';
			$res = mysql_query($query);
			if ($res===false) {
				echo "<p>Query failed: ($query) : ".mysql_error()."</p>";
			}
			$query = 'UPDATE `imas_fakeusers` SET homelayout = CONCAT(\'|0,1,2\',SUBSTR(homelayout,7))';
			$res = mysql_query($query);
			if ($res===false) {
				echo "<p>Query failed: ($query) : ".mysql_error()."</p>";
			}
		...
		if ($last<35) {
			$query = 'ALTER TABLE `imas_fakeusers` ADD `listperpage` TINYINT(3) UNSIGNED NOT NULL DEFAULT \'20\'';
			 $res = mysql_query($query);
			 if ($res===false) {
			  echo "<p>Query failed: ($query) : ".mysql_error()."</p>";
			 }
		...
		if ($last < 67) {
			 $query = 'ALTER TABLE `imas_fakeusers` ADD `hasuserimg` TINYINT( 1 ) UNSIGNED NOT NULL DEFAULT \'0\'';
			 $res = mysql_query($query);
			 if ($res===false) {
			 	 echo "<p>Query failed: ($query) : ".mysql_error()."</p>";
			 }
			 ...
			 if (count($hasimg)>0) {
			 	 ...
			 	 $query = "UPDATE imas_fakeusers SET hasuserimg=1 WHERE id IN ($haslist)";
			 	 mysql_query($query);

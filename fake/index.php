...
$query = "SELECT homelayout FROM imas_fakeusers WHERE id='$userid'";
...
if ($showmessagesgadget) {
	...
	$query = "SELECT imas_msgs.id,imas_msgs.title,imas_msgs.senddate,imas_fakeusers.LastName,imas_fakeusers.FirstName,imas_msgs.courseid ";
	$query .= "FROM imas_msgs LEFT JOIN imas_fakeusers ON imas_fakeusers.id=imas_msgs.msgfrom WHERE ";
...
function printPostsGadget() {
	...
	$query .= "WHERE imas_forum_posts.userid=imas_fakeusers.id AND imas_forum_posts.id IN ($threadlist)";
	$result = mysql_query($query) or die("Query failed : $query " . mysql_error());

...
if ($cid==0) {
	...
	if (!isset($teacherid) && ($row[4]==0 || ($row[4]==1 && ($now<$row[1] || $now>$row[2])))) {
		...
		if ($groupid>0) {
			...
			$query = "SELECT i_u.LastName,i_u.FirstName FROM imas_stugroupmembers AS i_sg,imas_fakeusers AS i_u WHERE ";
		...
		$query .= "imas_wiki_revisions as i_w_r JOIN imas_fakeusers as i_u ON i_u.id=i_w_r.userid ";

...
<?php
	...
	$query = "SELECT imas_forum_posts.*,imas_fakeusers.FirstName,imas_fakeusers.LastName,imas_fakeusers.email,imas_fakeusers.hasuserimg,ifv.lastview from imas_forum_posts JOIN imas_fakeusers ";
	$query .= "ON imas_forum_posts.userid=imas_fakeusers.id LEFT JOIN (SELECT DISTINCT threadid,lastview FROM imas_forum_views WHERE userid='$userid') AS ifv ON ";
	...
	$query .= "imas_fakeusers.LastName,imas_fakeusers.FirstName,imas_forum_posts.postdate DESC";
	$result = mysql_query($query) or die("Query failed : $query " . mysql_error());
	$laststu = -1;
	$cnt = 0;

...
if (isset($_GET['modify'])) { //adding or modifying post
	...
	if (isset($_POST['subject'])) {  //form submitted
		...
		if ($sendemail) {
			$query = "SELECT iu.email FROM imas_fakeusers AS iu,imas_forum_subscriptions AS ifs WHERE ";
		...
		if (isset($notice) && $notice!='') {
			...
			if ($_GET['modify']=='reply') {
				...
				$query = "SELECT imas_forum_posts.*,imas_fakeusers.FirstName,imas_fakeusers.LastName from imas_forum_posts,imas_fakeusers ";
				$query .= "WHERE imas_forum_posts.userid=imas_fakeusers.id AND (imas_forum_posts.id='$threadid' OR imas_forum_posts.threadid='$threadid')";
				$result = mysql_query($query) or die("Query failed : $query " . mysql_error());

...
//(c) 2006 David Lippman
	...
	if (isset($_POST['message'])) {
		...
		if ($_GET['masssend']=="Message") {
			...
			$query = "SELECT FirstName,LastName,id FROM imas_fakeusers WHERE id IN ($tolist)";
			...
			if ($_POST['self']=="self") {
				...
				$query = "SELECT imas_fakeusers.id FROM imas_teachers,imas_fakeusers WHERE imas_teachers.courseid='$cid' AND imas_teachers.userid=imas_fakeusers.id ";
				$result = mysql_query($query) or die("Query failed : " . mysql_error());
				while ($row = mysql_fetch_row($result)) {
					$tolist[] = $row[0];
				}
			...
			//$query = "SELECT imas_fakeusers.FirstName,imas_fakeusers.LastName,imas_fakeusers.email,imas_fakeusers.id ";
			//$query .= "FROM imas_students,imas_fakeusers WHERE imas_students.courseid='$cid' AND imas_students.userid=imas_fakeusers.id";
			$tolist = "'".implode("','",explode(",",$_POST['tolist']))."'";
			$query = "SELECT FirstName,LastName,email,id FROM imas_fakeusers WHERE id IN ($tolist)";
			...
			$query = "SELECT FirstName,LastName,email FROM imas_fakeusers WHERE id='$userid'";
			$result = mysql_query($query) or die("Query failed : " . mysql_error());
			...
			if ($_POST['self']=="allt") {
				$query = "SELECT imas_fakeusers.FirstName,imas_fakeusers.LastName,imas_fakeusers.email,imas_fakeusers.id ";
				$query .= "FROM imas_teachers,imas_fakeusers WHERE imas_teachers.courseid='$cid' AND imas_teachers.userid=imas_fakeusers.id ";
				$query .= "AND imas_fakeusers.id<>'$userid'";
				$result = mysql_query($query) or die("Query failed : " . mysql_error());
				while ($row = mysql_fetch_row($result)) {
		...
		$query = "SELECT LastName,FirstName,SID FROM imas_fakeusers WHERE id IN ($tolist) ORDER BY LastName,FirstName";
		$result = mysql_query($query) or die("Query failed : " . mysql_error());

...
if ($myrights<20) {
	...
	if (!isset($_REQUEST['lib'])) {
		...
		if (isset($_POST['remove']) || isset($_POST['delete'])) {
			if (!isset($_POST['confirm'])) {
				...
				if (isset($_POST['remove'])) {
					$madechange = false;
					if ($isgrpadmin) {
						//$query = "DELETE imas_library_items FROM imas_library_items,imas_fakeusers WHERE ";
						//$query .= "imas_library_items.ownerid=imas_fakeusers.id AND imas_fakeusers.groupid='$groupid' AND ";
						//$query .= "imas_library_items.qsetid='$qsetid' AND imas_library_items.libid='$libid'";
						$query = "SELECT imas_library_items FROM imas_library_items,imas_fakeusers WHERE ";
						$query .= "imas_library_items.ownerid=imas_fakeusers.id AND imas_fakeusers.groupid='$groupid' AND ";
			...
			if ($isgrpadmin) {
				...
				$query .= "WHERE iq.id='$qsetid' AND iq.ownerid=imas_fakeusers.id AND (imas_fakeusers.groupid='$groupid' OR iq.userights>3)";
				$result = mysql_query($query) or die("Query failed :$query " . mysql_error());
				if (mysql_num_rows($result)>0) {
		...
		$query = "SELECT imas_library_items.ownerid,imas_fakeusers.groupid FROM imas_library_items,imas_fakeusers WHERE ";
		$query .= "imas_library_items.ownerid=imas_fakeusers.id AND imas_library_items.libid='$lib' AND imas_library_items.qsetid='$qsetid'";
		$result = mysql_query($query) or die("Query failed : $query " . mysql_error());
		...
		$query .= "imas_questionset.ownerid=imas_fakeusers.id AND imas_questionset.id='$qsetid'";	
		$result = mysql_query($query) or die("Query failed : $query " . mysql_error());

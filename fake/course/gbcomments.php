...
//(c) 2006 David Lippman
		...
		if (isset($_FILES['userfile']['name']) && $_FILES['userfile']['name']!='') {
			if (is_uploaded_file($_FILES['userfile']['tmp_name'])) {
				...
				while (($data = fgetcsv($handle, 4096, ",")) !== FALSE) {
					$query = "SELECT imas_fakeusers.id FROM imas_fakeusers,imas_students WHERE imas_fakeusers.id=imas_students.userid AND imas_students.courseid='$cid' AND ";
					if ($_POST['useridtype']==0) {
						$query .= "imas_fakeusers.SID='{$data[$usercol]}'";
					} else if ($_POST['useridtype']==1) {
						list($last,$first) = explode(',',$data[$usercol]);
						$first = str_replace(' ','',$first);
						$last = str_replace(' ','',$last);
						$query .= "imas_fakeusers.FirstName='$first' AND imas_fakeusers.LastName='$last'";
						//echo $query;
					} else {
						$query .= "0";
					}
	...
	if ($comtype=='stu') {
		$query = "SELECT i_s.id,iu.LastName,iu.FirstName,i_s.gbcomment FROM imas_students AS i_s, imas_fakeusers as iu ";
	} else if ($comtype=='instr') {
		$query = "SELECT i_s.id,iu.LastName,iu.FirstName,i_s.gbinstrcomment FROM imas_students AS i_s, imas_fakeusers as iu ";
	}

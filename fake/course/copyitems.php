...
if (!(isset($teacherid))) {
	...
	if (isset($_GET['action'])) {
		...
		if (mysql_num_rows($result)==0) {
			...
			if ($copyrights<2) {
				$oktocopy = 0;
				if ($copyrights==1) {
					$query = "SELECT imas_fakeusers.groupid FROM imas_courses,imas_fakeusers,imas_teachers WHERE imas_courses.id=imas_teachers.courseid ";
					$query .= "AND imas_teachers.userid=imas_fakeusers.id AND imas_courses.id='{$_POST['ctc']}'";
	...
	if ($oktocopy == 1) {
		if (isset($_GET['action']) && $_GET['action']=="copycalitems") {
			...
			$query = "SELECT ic.id,ic.name,ic.copyrights,iu.LastName,iu.FirstName,iu.email,it.userid,iu.groupid FROM imas_courses AS ic,imas_teachers AS it,imas_fakeusers AS iu,imas_groups WHERE ";
			...
			$query = "SELECT ic.id,ic.name,ic.copyrights,iu.LastName,iu.FirstName,iu.email,it.userid FROM imas_courses AS ic,imas_teachers AS it,imas_fakeusers AS iu WHERE ";

	...
	if (isset($_POST['remove'])) {
		if (isset($_GET['confirmed'])) {
			if ($_POST['remove']!='') {
				...
				if ($isadmin) {
					...
					$query = "SELECT imas_questionset.id FROM imas_questionset,imas_fakeusers WHERE ";
					$query .= "imas_questionset.id IN ($remlist) AND imas_questionset.ownerid=imas_fakeusers.id ";
					$query .= "AND imas_fakeusers.groupid='$groupid'";
				...
				if ($isgrpadmin) {
					$query = "SELECT imas_questionset.id FROM imas_questionset,imas_fakeusers WHERE imas_questionset.ownerid=imas_fakeusers.id AND imas_questionset.id IN ($remlist) AND imas_fakeusers.groupid='$groupid'";
		...
		if (isset($_POST['newowner'])) {
			if ($_POST['transfer']!='') {
				...
				if ($isgrpadmin) {
					$query = "SELECT imas_questionset.id FROM imas_questionset,imas_fakeusers WHERE imas_questionset.ownerid=imas_fakeusers.id AND imas_questionset.id IN ($translist) AND imas_fakeusers.groupid='$groupid'";
					$result = mysql_query($query) or die("Query failed : $query " . mysql_error());
					while ($row = mysql_fetch_row($result)) {
			...
			$query = "SELECT id,FirstName,LastName FROM imas_fakeusers WHERE rights>19 ORDER BY LastName,FirstName";
			$result = mysql_query($query) or die("Query failed : " . mysql_error());
			$i=0;
		...
		if (isset($_POST['qtochg'])) {
			if ($_POST['chglib']!='') {
				...
				if ($isgrpadmin) {
					$query = "SELECT imas_library_items.qsetid,imas_library_items.libid FROM imas_library_items,imas_fakeusers WHERE ";
					$query .= "imas_library_items.ownerid=imas_fakeusers.id AND (imas_fakeusers.groupid='$groupid' OR imas_library_items.libid=0)";
...
/*
				...
				foreach ($libarray as $qsetid) {
					if ($isgrpadmin) {
						$query = "SELECT imas_library_items.libid FROM imas_library_items,imas_fakeusers WHERE ";
						$query .= "imas_library_items.ownerid=imas_fakeusers.id AND imas_fakeusers.groupid='$groupid' ";
...
*/
			...
			if (!$isadmin && !$isgrpadmin) {
				...
				$query = "SELECT ili.libid FROM imas_library_items AS ili,imas_fakeusers WHERE ";
				$query .= "ili.ownerid=imas_fakeusers.id AND imas_fakeusers.groupid!='$groupid' AND ili.qsetid IN ($clist)";
				$result = mysql_query($query) or die("Query failed : $query" . mysql_error());
				while ($row = mysql_fetch_row($result)) {
					$locked[] = $row[0];	
				}
		...
		if (isset($_POST['qtochg'])) {
			...
			$now = time();
			$query = "SELECT firstName,lastName FROM imas_fakeusers WHERE id='$userid'";
			$result = mysql_query($query) or die("Query failed : $query" . mysql_error());
		...
		if (isset($_POST['qtochg'])) {
			if ($isgrpadmin) {
				$query = "SELECT imas_questionset.id FROM imas_questionset,imas_fakeusers WHERE imas_questionset.ownerid=imas_fakeusers.id AND imas_questionset.id IN ($chglist) AND imas_fakeusers.groupid='$groupid'";
				$result = mysql_query($query) or die("Query failed : $query " . mysql_error());
				$tochg = array();
				while ($row = mysql_fetch_row($result)) {
					$tochg[] = $row[0];
		...
		if (isset($_GET['confirmed'])) {
			if ($isgrpadmin) {
				$query = "SELECT imas_questionset.id FROM imas_questionset,imas_fakeusers WHERE ";
				$query .= "imas_questionset.ownerid=imas_fakeusers.id AND imas_fakeusers.groupid='$groupid' AND ";
		...
		if (isset($_POST['newowner'])) {
			...
			if ($isgrpadmin) {
				$query = "SELECT imas_questionset.id FROM imas_questionset,imas_fakeusers WHERE imas_questionset.ownerid=imas_fakeusers.id AND imas_questionset.id='{$_GET['transfer']}' AND imas_fakeusers.groupid='$groupid'";
				$result = mysql_query($query) or die("Query failed : $query " . mysql_error());
				if (mysql_num_rows($result)>0) {
		...
		*/
		...
		$query .= "imas_questionset.qtype,imas_fakeusers.firstName,imas_fakeusers.lastName,imas_fakeusers.groupid,imas_library_items.libid,imas_library_items.junkflag, imas_library_items.id AS libitemid ";
		$query .= "FROM imas_questionset,imas_library_items,imas_fakeusers WHERE imas_questionset.deleted=0 AND $searchlikes ";
		$query .= "imas_library_items.qsetid=imas_questionset.id AND imas_questionset.ownerid=imas_fakeusers.id ";
		...
		if ($isadmin) {
			...
			$query .= "AND (imas_fakeusers.groupid='$groupid' OR imas_questionset.userights>0) "; 
			$query .= "AND (imas_library_items.libid > 0 OR imas_fakeusers.groupid='$groupid')";
			if ($searchall==0) {
				$query .= " AND imas_library_items.libid IN ($llist)";
			}
			if ($searchmine==1) {

...
//(c) 2009 David Lippman
	...
	if (isset($_POST['submit'])) {
		...
		if (trim($_POST['newtutors'])!='') {
			...
			$existingsids = array();
			$query = "SELECT u.SID FROM imas_tutors as tut JOIN imas_fakeusers as u ON tut.userid=u.id WHERE tut.courseid='$cid'";
			$result = mysql_query($query) or die("Query failed : " . mysql_error());
			while ($row = mysql_fetch_row($result)) {
				$existingsids[] = $row[0];
			}
			//also don't want students enrolled as tutors
			$query = "SELECT u.SID FROM imas_students as stu JOIN imas_fakeusers as u ON stu.userid=u.id WHERE stu.courseid='$cid'";
			$result = mysql_query($query) or die("Query failed : " . mysql_error());
			while ($row = mysql_fetch_row($result)) {
				$existingsids[] = $row[0];
			}
			...
			if (count($sidstouse)>0) {
				...
				//check if SID exists
				$query = "SELECT id,SID FROM imas_fakeusers WHERE SID in ($tutsids)";
				$result = mysql_query($query) or die("Query failed : " . mysql_error());
	...
	$tutorlist = array();
	$i = 0;
	$query = "SELECT tut.id,u.LastName,u.FirstName,tut.section FROM imas_tutors as tut JOIN imas_fakeusers as u ON tut.userid=u.id WHERE tut.courseid='$cid' ORDER BY u.LastName,u.FirstName";
	$result = mysql_query($query) or die("Query failed : " . mysql_error());
	while ($row = mysql_fetch_row($result)) {

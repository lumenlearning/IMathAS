	...
	if (isset($_GET['addgrpset'])) {
		...
		if ($_POST['addtogrpid']=='--new--') {
			...
			if (count($alreadygroupedstu)>0) {
				...
				$query = "SELECT FirstName,LastName FROM imas_fakeusers WHERE id IN ($stulist) ORDER BY LastName, FirstName";
				$result = mysql_query($query) or die("Query failed : $query:" . mysql_error());
				while ($row = mysql_fetch_row($result)) {
					echo $row[1].', '.$row[0].'<br/>';
		...
		if (isset($_GET['confirm'])) {
			...
			$query = "SELECT LastName, FirstName FROM imas_fakeusers WHERE id='{$_GET['remove']}'";
			$result = mysql_query($query) or die("Query failed : " . mysql_error());
		...
		$stunames = array();
		$hasuserimg = array();
		$query = "SELECT iu.id,iu.FirstName,iu.LastName,iu.hasuserimg FROM imas_fakeusers AS iu JOIN imas_students ON iu.id=imas_students.userid WHERE imas_students.courseid='$cid'";
		$result = mysql_query($query) or die("Query failed : " . mysql_error());
		while ($row = mysql_fetch_row($result)) {

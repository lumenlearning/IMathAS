	...
	if (isset($_POST['remove'])) {
		...
		if (isset($_POST['newowner'])) {
			if ($_POST['transfer']!='') {
				...
				$query = "SELECT groupid FROM imas_fakeusers WHERE id='{$_POST['newowner']}'";
				$result = mysql_query($query) or die("Query failed : " . mysql_error());
		...
		if (isset($_POST['newowner'])) {
			...
			//$query = "UPDATE imas_libraries,imas_fakeusers SET imas_libraries.ownerid='{$_POST['newowner']}'";
			//$query .= ",imas_libraries.groupid=imas_fakeusers.groupid WHERE imas_libraries.ownerid=imas_fakeusers.id AND ";

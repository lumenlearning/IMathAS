...
if (!(isset($teacherid))) { // loaded by a NON-teacher
	...
	for ($i = 0; $i < count($items); $i++) {
		...
		for ($j=0;$j<count($subs);$j++) {
			$query = "SELECT imas_questions.questionsetid,imas_questionset.description,imas_questionset.userights,imas_questionset.ownerid,imas_questionset.qtype,imas_questions.points,imas_questions.withdrawn,imas_questionset.extref,imas_fakeusers.groupid,imas_questions.showhints FROM imas_questions,imas_questionset,imas_fakeusers ";
			$query .= "WHERE imas_questions.id='{$subs[$j]}' AND imas_questionset.id=imas_questions.questionsetid AND imas_questionset.ownerid=imas_fakeusers.id ";
			$result = mysql_query($query) or die("Query failed : " . mysql_error());
	...
	if ($sessiondata['selfrom'.$aid]=='lib') { //selecting from libraries
		...
		if (isset($sessiondata['aidstolist'.$aid])) { //list questions
			...
			foreach ($sessiondata['aidstolist'.$aid] as $aidq) {
				$query = "SELECT imas_questions.id,imas_questionset.id,imas_questionset.description,imas_questionset.qtype,imas_questionset.ownerid,imas_questionset.userights,imas_questionset.extref,imas_fakeusers.groupid FROM imas_questionset,imas_questions,imas_fakeusers";
				$query .= " WHERE imas_questionset.id=imas_questions.questionsetid AND imas_questionset.ownerid=imas_fakeusers.id AND imas_questions.assessmentid='$aidq'";
				$result = mysql_query($query) or die("Query failed : " . mysql_error());

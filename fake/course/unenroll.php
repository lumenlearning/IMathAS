...
ini_set("max_execution_time", "600");
	...
	if (isset($_GET['confirmed'])) { //do unenroll
		...
		if ($_GET['uid']=="all") {
			$query = "SELECT iu.LastName,iu.FirstName,iu.SID FROM imas_fakeusers AS iu JOIN imas_students ON iu.id=imas_students.userid WHERE imas_students.courseid='$cid'";
			$resultUserList = mysql_query($query) or die("Query failed : " . mysql_error());
		} else if ($_GET['uid']=="selected") {
			if (count($_POST['checked'])>0) {
				$ulist = "'".implode("','",$_POST['checked'])."'";
				$query = "SELECT LastName,FirstName,SID FROM imas_fakeusers WHERE id IN ($ulist)";
				$resultUserList = mysql_query($query) or die("Query failed : " . mysql_error());
				$query = "SELECT COUNT(id) FROM imas_students WHERE courseid='{$_GET['cid']}'";
				$result = mysql_query($query) or die("Query failed : " . mysql_error());
			...
			$query = "SELECT FirstName,LastName,SID FROM imas_fakeusers WHERE id='{$_GET['uid']}'";
			$result = mysql_query($query) or die("Query failed : " . mysql_error());

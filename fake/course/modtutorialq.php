...
if (isset($_POST['text'])) {
	...
	if ($id=='new') {
		...
		$isok = true;
		if ($isgrpadmin) {
			$query = "SELECT iq.id FROM imas_questionset AS iq,imas_fakeusers ";
			$query .= "WHERE iq.id='{$_GET['id']}' AND iq.ownerid=imas_fakeusers.id AND (imas_fakeusers.groupid='$groupid' OR iq.userights>2)";
			$result = mysql_query($query) or die("Query failed :$query " . mysql_error());
			if (mysql_num_rows($result)==0) {
				$isok = false;
			}
			//$query = "UPDATE imas_questionset AS iq,imas_fakeusers SET iq.description='{$_POST['description']}',iq.author='{$_POST['author']}',iq.userights='{$_POST['userights']}',";
		...
		if (!$isadmin && !$isgrpadmin) {  //check is owner or is allowed to modify
			$query = "SELECT iq.id FROM imas_questionset AS iq,imas_fakeusers ";
			$query .= "WHERE iq.id='{$_GET['id']}' AND iq.ownerid=imas_fakeusers.id AND (iq.ownerid='$userid' OR (iq.userights=3 AND imas_fakeusers.groupid='$groupid') OR iq.userights>3)";
			$result = mysql_query($query) or die("Query failed :$query " . mysql_error());
			if (mysql_num_rows($result)==0) {
				$isok = false;
			}
	...
	if ($isgrpadmin) {
		$query = "SELECT ili.libid FROM imas_library_items AS ili,imas_fakeusers WHERE ili.ownerid=imas_fakeusers.id ";
		$query .= "AND (imas_fakeusers.groupid='$groupid' OR ili.libid=0) AND ili.qsetid='$qsetid'";
	} else {
		/*
...
if (isset($_GET['id']) && $_GET['id']!='new') {
	...
	$query = "SELECT imas_questionset.*,imas_fakeusers.groupid FROM imas_questionset,imas_fakeusers WHERE ";
	$query .= "imas_questionset.ownerid=imas_fakeusers.id AND imas_questionset.id='{$_GET['id']}'";
	$result = mysql_query($query) or die("Query failed : $query" . mysql_error());
	...
	if (isset($_GET['template'])) {
		$query = "SELECT deflib,usedeflib FROM imas_fakeusers WHERE id='$userid'";
		$result = mysql_query($query) or die("Query failed : $query " . mysql_error());
		...
		$query = "SELECT qrightsdef FROM imas_fakeusers WHERE id='$userid'";
		$result = mysql_query($query) or die("Query failed : " . mysql_error());
		$line['userights'] = mysql_result($result,0,0);
	
	} else {
		if ($isgrpadmin) {
			$query = "SELECT DISTINCT ili.libid FROM imas_library_items AS ili,imas_fakeusers WHERE ili.ownerid=imas_fakeusers.id ";
			$query .= "AND imas_fakeusers.groupid='$groupid' AND ili.qsetid='{$_GET['id']}'";
		} else {

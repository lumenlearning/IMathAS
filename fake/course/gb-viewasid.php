...
//(c) 2007 David Lippman
	...
	if ($links==0) { //View/Edit full assessment
		...
		$query = "SELECT imas_fakeusers.FirstName,imas_fakeusers.LastName,imas_students.timelimitmult FROM imas_fakeusers JOIN imas_students ON imas_fakeusers.id=imas_students.userid WHERE imas_fakeusers.id='{$_GET['uid']}' AND imas_students.courseid='$cid'";
		$result = mysql_query($query) or die("Query failed : $query " . mysql_error());
		$row = mysql_fetch_row($result);
		...
		if (($isteacher || $istutor) && !isset($_GET['lastver']) && !isset($_GET['reviewver'])) {
			if ($line['agroupid']>0) {
				$q2 = "SELECT i_u.LastName,i_u.FirstName FROM imas_assessment_sessions AS i_a_s,imas_fakeusers AS i_u WHERE ";
		...
		$query = "SELECT FirstName,LastName FROM imas_fakeusers WHERE id='{$_GET['uid']}'";
		$result = mysql_query($query) or die("Query failed : $query " . mysql_error());
		$row = mysql_fetch_row($result);
		echo "<h3>{$row[1]}, {$row[0]}</h3>\n";

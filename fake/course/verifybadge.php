...
if (!empty($_GET['userid'])) {
	...
	$query = "SELECT SID FROM imas_fakeusers WHERE id='$userid'";
...
function print_assertation($cid, $badgetext, $badgename, $descr, $userid, $email) {
	...
	/*$query = "SELECT imas_courses.name AS cname, imas_fakeusers.LastName, imas_fakeusers.FirstName, imas_fakeusers.email, imas_groups.name FROM imas_courses JOIN imas_teachers ON imas_courses.id=imas_teachers.courseid ";
	$query .= "JOIN imas_fakeusers ON imas_teachers.userid=imas_fakeusers.id LEFT JOIN imas_groups ON imas_fakeusers.groupid=imas_groups.id WHERE imas_courses.id='$cid' LIMIT 1";
	$result = mysql_query($query) or die("Query failed : " . mysql_error());
	if (mysql_num_rows($result)==0) {
		$org = ' ';
		$email = ' ';

...
function gbtable() {
	...
	//Pull student data
	$ln = 1;
	$query = "SELECT imas_fakeusers.id,imas_fakeusers.SID,imas_fakeusers.FirstName,imas_fakeusers.LastName,imas_fakeusers.SID,imas_fakeusers.email,imas_students.section,imas_students.code,imas_students.locked,imas_students.timelimitmult,imas_students.lastaccess,imas_fakeusers.hasuserimg ";
	$query .= "FROM imas_fakeusers,imas_students WHERE imas_fakeusers.id=imas_students.userid AND imas_students.courseid='$cid' ";
	//$query .= "FROM imas_fakeusers,imas_teachers WHERE imas_fakeusers.id=imas_teachers.userid AND imas_teachers.courseid='$cid' ";
	//if (!$isteacher && !isset($tutorid)) {$query .= "AND imas_fakeusers.id='$userid' ";}
	if ($limuser>0) { $query .= "AND imas_fakeusers.id='$limuser' ";}
	if ($secfilter!=-1) {
	...
	if (isset($timefilter)) {
		...
		$query .= "AND imas_fakeusers.lastaccess>$tf ";
	}
	if (isset($lnfilter) && $lnfilter!='') {
		$query .= "AND imas_fakeusers.LastName LIKE '$lnfilter%' ";
	}
	if ($isdiag) {
		$query .= "ORDER BY imas_fakeusers.email,imas_fakeusers.LastName,imas_fakeusers.FirstName";

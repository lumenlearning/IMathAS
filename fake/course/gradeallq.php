...
//(c) 2007 David Lippman
	...
	if (isset($_GET['update'])) {
		...
		$query = "SELECT imas_fakeusers.LastName,imas_fakeusers.FirstName,imas_assessment_sessions.* FROM imas_fakeusers,imas_assessment_sessions ";
		$query .= "WHERE imas_assessment_sessions.userid=imas_fakeusers.id AND imas_assessment_sessions.assessmentid='$aid' ";
	...
	if ($isgroup>0) {
		...
		$groupmembers = array();
		$query = "SELECT isg.stugroupid,iu.LastName,iu.FirstName FROM imas_stugroupmembers AS isg JOIN imas_fakeusers as iu ON isg.userid=iu.id WHERE isg.stugroupid IN ($grplist) ORDER BY iu.LastName,iu.FirstName";
...
<?php
	...
	if ($page!=-1) {
		$stulist = array();
		$query = "SELECT imas_fakeusers.LastName,imas_fakeusers.FirstName,imas_assessment_sessions.* FROM imas_fakeusers,imas_assessment_sessions,imas_students ";
		$query .= "WHERE imas_assessment_sessions.userid=imas_fakeusers.id AND imas_students.userid=imas_fakeusers.id AND imas_students.courseid='$cid' AND imas_assessment_sessions.assessmentid='$aid' ";
		$query .= "ORDER BY imas_fakeusers.LastName,imas_fakeusers.FirstName";
		$result = mysql_query($query) or die("Query failed : $query: " . mysql_error());
		while ($row = mysql_fetch_row($result)) {
			$stulist[] = $row[0].', '.$row[1];

...
if (!isset($teacherid)) { // loaded by a NON-teacher
	...
	if (isset($_GET['assigncode'])) {
		...
		if (isset($_POST['submit'])) {
			...
			$query = "SELECT imas_students.id,imas_fakeusers.FirstName,imas_fakeusers.LastName,imas_students.section,imas_students.code ";
			$query .= "FROM imas_students,imas_fakeusers WHERE imas_students.courseid='$cid' AND imas_students.userid=imas_fakeusers.id ";
			$query .= "ORDER BY imas_fakeusers.LastName,imas_fakeusers.FirstName";
		...
		if (isset($_POST['username'])) {
			$query = "SELECT id FROM imas_fakeusers WHERE SID='{$_POST['username']}'";
		...
		if (isset($_POST['SID'])) {
			$query = "SELECT id FROM imas_fakeusers WHERE SID='{$_POST['SID']}'";
			$result = mysql_query($query) or die("Query failed : " . mysql_error());
			if (mysql_num_rows($result)>0) {
				$overwriteBody = 1;
				...
				$md5pw = md5($_POST['pw1']);
				$query = "INSERT INTO imas_fakeusers (SID, password, rights, FirstName, LastName, email, msgnotify) ";
		...
		if (count($_POST['checked'])>0) {
			...
			$query = "SELECT imas_fakeusers.FirstName,imas_fakeusers.LastName,imas_fakeusers.email ";
			$query .= "FROM imas_students JOIN imas_fakeusers ON imas_students.userid=imas_fakeusers.id WHERE imas_students.courseid='$cid' AND imas_fakeusers.id IN ($ulist)";
			$query .= "ORDER BY imas_fakeusers.LastName,imas_fakeusers.FirstName";
			$result = mysql_query($query) or die("Query failed : " . mysql_error());
			$stuemails = array();
		...
		if (isset($_POST['firstname'])) {
			...
			$updateusername = true;
			$query = "SELECT id FROM imas_fakeusers WHERE SID='$un'";
			$result = mysql_query($query) or die("Query failed : " . mysql_error());
			if (mysql_num_rows($result)>0) {
				$updateusername = false;
			}
			$query = "UPDATE imas_fakeusers SET FirstName='{$_POST['firstname']}',LastName='{$_POST['lastname']}',email='{$_POST['email']}'";
			if ($updateusername) {
				$query .= ",SID='$un'";
			}
			...
			if ($chguserimg != '') {
				$query = "UPDATE imas_fakeusers SET $chguserimg WHERE id='{$_GET['uid']}'";
				mysql_query($query) or die("Query failed : " . mysql_error());
			}
			...
			$query = "SELECT imas_fakeusers.*,imas_students.code,imas_students.section,imas_students.locked,imas_students.timelimitmult,imas_students.hidefromcourselist FROM imas_fakeusers,imas_students ";
			$query .= "WHERE imas_fakeusers.id=imas_students.userid AND imas_fakeusers.id='{$_GET['uid']}' AND imas_students.courseid='$cid'";
			$result = mysql_query($query) or die("Query failed : " . mysql_error());
		...
		if (isset($_GET['confirmed'])) {
			...
			$query = "UPDATE imas_fakeusers SET password='$newpw' WHERE id='{$_GET['uid']}'";
			mysql_query($query) or die("Query failed : " . mysql_error());
		} else {
			$curBreadcrumb = "$breadcrumbbase <a href=\"course.php?cid=$cid\"> $coursename</a>\n"; 
		...
		$query = "SELECT imas_students.id,imas_students.userid,imas_fakeusers.FirstName,imas_fakeusers.LastName,imas_fakeusers.email,imas_fakeusers.SID,imas_students.lastaccess,imas_students.section,imas_students.code,imas_students.locked,imas_fakeusers.hasuserimg ";
		$query .= "FROM imas_students,imas_fakeusers WHERE imas_students.courseid='$cid' AND imas_students.userid=imas_fakeusers.id ";
		if ($secfilter>-1) {
			$query .= "AND imas_students.section='$secfilter' ";			
		}
		if ($hassection) {
			$query .= "ORDER BY imas_students.section,imas_fakeusers.LastName,imas_fakeusers.FirstName";
		} else {
			$query .= "ORDER BY imas_fakeusers.LastName,imas_fakeusers.FirstName";
		}

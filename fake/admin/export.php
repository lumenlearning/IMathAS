...
if (!(isset($teacherid)) && $myrights<75) {
	...
	if (isset($search)) {
		if ($isadmin) {
			...
			$query = "SELECT DISTINCT imas_questionset.id,imas_questionset.description,imas_questionset.qtype ";
			$query .= "FROM imas_questionset,imas_library_items,imas_fakeusers WHERE imas_questionset.description LIKE '%$safesearch%' ";
			$query .= "AND imas_questionset.ownerid=imas_fakeusers.id ";

...
switch($_GET['action']) {
	...
	case "chgrights":  
		...
		$query = "UPDATE imas_fakeusers SET rights='{$_POST['newrights']}'";
		if ($myrights == 100) {
	...
	case "resetpwd":
		...
		$query = "UPDATE imas_fakeusers SET password='$md5pw' WHERE id='{$_GET['id']}'";
		if ($myrights < 100) { $query .= " AND groupid='$groupid' AND rights<100"; }
		mysql_query($query) or die("Query failed : " . mysql_error());
		break;
	case "deladmin":
		if ($myrights < 75) { echo "You don't have the authority for this action"; break;}
		$query = "DELETE FROM imas_fakeusers WHERE id='{$_GET['id']}'";
		if ($myrights < 100) { $query .= " AND groupid='$groupid' AND rights<100"; }
		mysql_query($query) or die("Query failed : " . mysql_error());
		...
		break;
	case "chgpwd":
		$query = "SELECT password FROM imas_fakeusers WHERE id = '$userid'";
		$result = mysql_query($query) or die("Query failed : " . mysql_error());
		$line = mysql_fetch_array($result, MYSQL_ASSOC);
	
		if ((md5($_POST['oldpw'])==$line['password']) && ($_POST['newpw1'] == $_POST['newpw2'])) {
			$md5pw =md5($_POST['newpw1']);
			$query = "UPDATE imas_fakeusers SET password='$md5pw' WHERE id='$userid'";
			mysql_query($query) or die("Query failed : " . mysql_error()); 
		} else {
	...
	case "newadmin":
		...
		$query = "SELECT id FROM imas_fakeusers WHERE SID = '{$_POST['adminname']}';";
		$result = mysql_query($query) or die("Query failed : " . mysql_error());
		...
		$query = "INSERT INTO imas_fakeusers (SID,password,FirstName,LastName,rights,email,groupid,homelayout) VALUES ('{$_POST['adminname']}','$md5pw','{$_POST['firstname']}','{$_POST['lastname']}','{$_POST['newrights']}','{$_POST['email']}','$newgroup','$homelayout');";
		$result = mysql_query($query) or die("Query failed : " . mysql_error());
		break;
	case "logout":
	...
	case "delete":
		...
		if (isset($CFG['GEN']['doSafeCourseDelete']) && $CFG['GEN']['doSafeCourseDelete']==true) {
			...
			if ($myrights < 75) {
				...
				$query = "SELECT imas_courses.id FROM imas_courses,imas_fakeusers WHERE imas_courses.id='{$_GET['id']}' AND imas_courses.ownerid=imas_fakeusers.id AND imas_fakeusers.groupid='$groupid'";
				$result = mysql_query($query) or die("Query failed : " . mysql_error());
				if (mysql_num_rows($result)>0) {
					$oktodel = true;
				}
	...
	case "remteacher":
		...
		foreach ($tids as $tid) {
			if ($myrights < 100) {
				$query = "SELECT imas_teachers.id FROM imas_teachers,imas_fakeusers WHERE imas_teachers.id='$tid' AND imas_teachers.userid=imas_fakeusers.id AND imas_fakeusers.groupid='$groupid'";
				$result = mysql_query($query) or die("Query failed : " . mysql_error());
				if (mysql_num_rows($result)>0) {
				...
				//$query = "DELETE imas_teachers FROM imas_fakeusers,imas_teachers WHERE imas_teachers.id='{$_GET['tid']}' ";
				//$query .= "AND imas_teachers.userid=imas_fakeusers.id AND imas_fakeusers.groupid='$groupid'";
			} else {
				$query = "DELETE FROM imas_teachers WHERE id='$tid'";
				mysql_query($query) or die("Query failed : " . mysql_error());
			}
	...
	case "addteacher":
		if ($myrights < 40) { echo "You don't have the authority for this action"; break;}
		if ($myrights < 100) {
			$query = "SELECT imas_fakeusers.groupid FROM imas_fakeusers,imas_courses WHERE imas_courses.ownerid=imas_fakeusers.id AND imas_courses.id='{$_GET['cid']}'";
			$result = mysql_query($query) or die("Query failed : " . mysql_error());
	...
	case "transfer":
		...
		if ($myrights==75) {
			...
			//$query = "UPDATE imas_courses,imas_fakeusers SET imas_courses.ownerid='{$_POST['newowner']}' WHERE ";
			//$query .= "imas_courses.id='{$_GET['id']}' AND imas_courses.ownerid=imas_fakeusers.id AND imas_fakeusers.groupid='$groupid'";
		} else {
			mysql_query($query) or die("Query failed : " . mysql_error());
			$exec = true;
		}
	...
	case "deloldusers":
		...
		require_once("../includes/filehandler.php");
		if ($who=="students") {
			$query = "SELECT id FROM imas_fakeusers WHERE  lastaccess<$old AND (rights=0 OR rights=10)";
			$result = mysql_query($query) or die("Query failed : " . mysql_error());
			while ($row = mysql_fetch_row($result)) {
				$uid = $row[0];
			...
			$query = "DELETE FROM imas_fakeusers WHERE lastaccess<$old AND (rights=0 OR rights=10)";
		} else if ($who=="all") {
			$query = "DELETE FROM imas_fakeusers WHERE lastaccess<$old AND rights<100";
		}
		mysql_query($query) or die("Query failed : " . mysql_error());
		break;
	case "addgroup":
	...
	case "delgroup":
		...
		mysql_query($query) or die("Query failed : " . mysql_error());
		$query = "UPDATE imas_fakeusers SET groupid=0 WHERE groupid='{$_GET['id']}'";
		mysql_query($query) or die("Query failed : " . mysql_error());
	...
	case "modltidomaincred":
		if ($myrights <100) { echo "You don't have the authority for this action"; break;}
		if ($_GET['id']=='new') {
			$query = "INSERT INTO imas_fakeusers (email,FirstName,LastName,SID,password,rights,groupid) VALUES ";
			$query .= "('{$_POST['ltidomain']}','{$_POST['ltidomain']}','LTIcredential','{$_POST['ltikey']}',";
			$query .= "'{$_POST['ltisecret']}','{$_POST['createinstr']}','{$_POST['groupid']}')";
		} else {
			$query = "UPDATE imas_fakeusers SET email='{$_POST['ltidomain']}',FirstName='{$_POST['ltidomain']}',LastName='LTIcredential',";
	...
	case "removediag";
		...
		$query = "SELECT imas_fakeusers.id,imas_fakeusers.groupid FROM imas_fakeusers JOIN imas_diags ON imas_fakeusers.id=imas_diags.ownerid AND imas_diags.id='{$_GET['id']}'";
		$result = mysql_query($query) or die("Query failed : " . mysql_error());
		$row = mysql_fetch_row($result);

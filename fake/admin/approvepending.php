...
if (isset($_GET['go'])) {
	if (isset($_POST['skip'])) {
		...
		$query = "UPDATE imas_fakeusers SET rights=10 WHERE id='{$_POST['id']}'";
		...
		$query = "UPDATE imas_fakeusers SET rights=40,groupid=$group WHERE id='{$_POST['id']}'";
		mysql_query($query) or die("Query failed : " . mysql_error());
		
		$query = "SELECT FirstName,SID,email FROM imas_fakeusers WHERE id='{$_POST['id']}'";
...
require("../header.php");
$query = "SELECT id,SID,LastName,FirstName,email FROM imas_fakeusers WHERE rights=0 OR rights=12 LIMIT 1 OFFSET $offset";
$result = mysql_query($query) or die("Query failed : " . mysql_error());

...
if (!(isset($teacherid)) && $myrights<100) {
	...
	if (isset($_POST['process'])) {
		...
		while (($data = fgetcsv($handle,2096))!==false) {
			...
			$query = "SELECT id FROM imas_fakeusers WHERE SID='$arr[0]'";
			...
			if (mysql_num_rows($result)>0) {
				...
				$query = "INSERT INTO imas_fakeusers (SID,FirstName,LastName,email,rights,password) VALUES ('$arr[0]','$arr[1]','$arr[2]','$arr[3]',10,'$pw')";
		...
		if ($isadmin) {
			$query = "SELECT imas_courses.id,imas_courses.name,imas_fakeusers.LastName,imas_fakeusers.FirstName FROM imas_courses,imas_fakeusers ";
			$query .= "WHERE imas_fakeusers.id=imas_courses.ownerid ";

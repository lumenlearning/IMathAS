		...
		foreach ($checked as $qn) {
			...
			if (isset($exists[$qdata[$qn]['uqid']]) && $_POST['merge']==1) {
				...
				if ($qdata[$qn]['lastmod']>$adddate[$qsetid]) { //only update modified questions - should add check for different lastmoddates
					if ($isgrpadmin) {
						$query = "SELECT imas_questionset.id FROM imas_questionset,imas_fakeusers WHERE WHERE imas_questionset.id='$qsetid' AND imas_questionset.ownerid=imas_fakeusers.id AND imas_fakeusers.groupid='$groupid'";
						$result = mysql_query($query) or die("Query failed : " . mysql_error());
						if (mysql_num_rows($result)>0) {
						//$query = "UPDATE imas_questionset,imas_fakeusers SET imas_questionset.description='{$qdata[$qn]['description']}',imas_questionset.author='{$qdata[$qn]['author']}',";
						...
						//$query .= " AND imas_questionset.ownerid=imas_fakeusers.id AND imas_fakeusers.groupid='$groupid'";

...
switch($_GET['action']) {
	...
	case "chgteachers":
		...
		if ($myrights<100) {
			$query = "SELECT id,FirstName,LastName,rights FROM imas_fakeusers WHERE rights>19 AND rights<>76 AND groupid='$groupid' ORDER BY LastName;";
		} else if ($myrights==100) {
			$query = "SELECT id,FirstName,LastName,rights FROM imas_fakeusers WHERE rights>19 AND rights<>76 ORDER BY LastName;";
		}
		$result = mysql_query($query) or die("Query failed : " . mysql_error());
	...
	case "listltidomaincred":
		...
		$query = "SELECT id,email,SID,rights FROM imas_fakeusers WHERE rights=11 OR rights=76";
		$result = mysql_query($query) or die("Query failed : " . mysql_error());
		while ($row = mysql_fetch_row($result)) {
	...
	case "modltidomaincred":
		...
		echo "<h3>Modify LTI Domain Credentials</h3>\n";
		echo '</div>';
		$query = "SELECT id,email,SID,password,rights FROM imas_fakeusers WHERE id='{$_GET['id']}'";
		$result = mysql_query($query) or die("Query failed : " . mysql_error());

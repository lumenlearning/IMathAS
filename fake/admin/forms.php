...
switch($_GET['action']) {
	...
	case "newadmin":
		...
		if ($_GET['action'] == "newadmin") {
			...
			$query = "SELECT FirstName,LastName,rights,groupid FROM imas_fakeusers WHERE id='{$_GET['id']}'";
			$result = mysql_query($query) or die("Query failed : " . mysql_error());
	...
	case "addcourse":
		...
		if (isset($CFG['CPS']['templateoncreate']) && $_GET['action']=='addcourse' ) {
			...
			$query = "SELECT ic.id,ic.name,ic.copyrights FROM imas_courses AS ic JOIN imas_fakeusers AS iu ON ic.ownerid=iu.id WHERE ";
	...
	case "chgteachers":
		...
		$query = "SELECT imas_fakeusers.FirstName,imas_fakeusers.LastName,imas_teachers.id,imas_teachers.userid ";
		$query .= "FROM imas_fakeusers,imas_teachers WHERE imas_teachers.courseid='{$_GET['id']}' AND " ;
		$query .= "imas_teachers.userid=imas_fakeusers.id ORDER BY imas_fakeusers.LastName;";
		$result = mysql_query($query) or die("Query failed : " . mysql_error());
		...
		if ($myrights<100) {
			$query = "SELECT id,FirstName,LastName,rights FROM imas_fakeusers WHERE rights>19 AND (rights<76 or rights>78) AND groupid='$groupid' ORDER BY LastName;";
		} else if ($myrights==100) {
			$query = "SELECT id,FirstName,LastName,rights FROM imas_fakeusers WHERE rights>19 AND (rights<76 or rights>78) ORDER BY LastName;";
		}
		$result = mysql_query($query) or die("Query failed : " . mysql_error());
	...
	case "transfer":
		...
		$query = "SELECT id,FirstName,LastName FROM imas_fakeusers WHERE rights>19";
		if ($myrights < 100) {
	...
	case "listltidomaincred":
		...
		$query = "SELECT id,email,SID,rights FROM imas_fakeusers WHERE rights=11 OR rights=76 OR rights=77";
		$result = mysql_query($query) or die("Query failed : " . mysql_error());
		while ($row = mysql_fetch_row($result)) {
	...
	case "modltidomaincred":
		...
		echo "<h3>Modify LTI Domain Credentials</h3>\n";
		echo '</div>';
		$query = "SELECT id,email,SID,password,rights,groupid FROM imas_fakeusers WHERE id='{$_GET['id']}'";
		$result = mysql_query($query) or die("Query failed : " . mysql_error());

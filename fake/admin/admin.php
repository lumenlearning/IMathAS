...
if ($myrights < 40) { 
 ...
 //data processing for COURSES block 
	$query = "SELECT imas_courses.id,imas_courses.ownerid,imas_courses.name,imas_courses.available,imas_fakeusers.FirstName,imas_fakeusers.LastName FROM imas_courses,imas_fakeusers ";
	$query .= "WHERE imas_courses.ownerid=imas_fakeusers.id ";
	...
	if ($myrights >= 75 && $showcourses>0) {
		...
		$query .= " ORDER BY imas_fakeusers.LastName,imas_courses.name";
	} else {
	...
	if ($myrights==75) {
		$query = "SELECT id,LastName,FirstName,SID FROM imas_fakeusers WHERE rights>10 AND groupid='$groupid' ORDER BY LastName,FirstName";
	} else if ($myrights==100) {
		$query = "SELECT id,LastName,FirstName,SID FROM imas_fakeusers WHERE rights>10 ORDER BY LastName,FirstName";
	}
	$result = mysql_query($query) or die("Query failed : $query" . mysql_error());
	$i=0;
	...
	if ($myrights>=60) {
		$query = "SELECT d.id,d.name,d.public FROM imas_diags as d JOIN imas_fakeusers AS u ON u.id=d.ownerid";
		if ($myrights<75) {
	...
	if ($myrights < 100) {
		...
		$query = "SELECT id,SID,FirstName,LastName,rights,lastaccess FROM imas_fakeusers WHERE rights > 10 AND groupid='$groupid' ORDER BY LastName";
	} else {
		if ($showusers==-1) {
			$page_userBlockTitle = "Pending Users";
			$query = "SELECT id,SID,FirstName,LastName,email,rights,lastaccess FROM imas_fakeusers WHERE rights=0 OR rights=12 ORDER BY LastName";
		} else if (is_numeric($showusers)) {
			$page_userBlockTitle = "Group Users";
			$query = "SELECT id,SID,FirstName,LastName,email,rights,lastaccess FROM imas_fakeusers WHERE rights > 10 AND groupid='$showusers' ORDER BY LastName";
		} else {
			$page_userBlockTitle = "All Users - $showusers";
			$query = "SELECT id,SID,FirstName,LastName,email,rights,lastaccess FROM imas_fakeusers WHERE substring(LastName,1,1)='$showusers' ORDER BY LastName";
		}
	}

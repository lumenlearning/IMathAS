...
if (!isset($_POST['embedpostback'])) {
		...
		if (isset($_POST['grpsubmit'])) {
			...
			for ($i=1;$i<$testsettings['groupmax'];$i++) {
				if (isset($_POST['user'.$i]) && $_POST['user'.$i]!=0) {
					$query = "SELECT password,LastName,FirstName FROM imas_fakeusers WHERE id='{$_POST['user'.$i]}'";
					$result = mysql_query($query) or die("Query failed : $query:" . mysql_error());
			...
			$curgrp = array();
			$query = "SELECT imas_fakeusers.id,imas_fakeusers.FirstName,imas_fakeusers.LastName FROM imas_fakeusers,imas_stugroupmembers WHERE ";
			$query .= "imas_fakeusers.id=imas_stugroupmembers.userid AND imas_stugroupmembers.stugroupid='{$sessiondata['groupid']}' ORDER BY imas_fakeusers.LastName,imas_fakeusers.FirstName";
			$result = mysql_query($query) or die("Query failed : $query;  " . mysql_error());
			while ($row = mysql_fetch_row($result)) {
				$curgrp[0] = $row[0];
			...
			$query = "SELECT imas_fakeusers.id,imas_fakeusers.FirstName,imas_fakeusers.LastName FROM imas_fakeusers,imas_students ";
			$query .= "WHERE imas_fakeusers.id=imas_students.userid AND imas_students.courseid='{$testsettings['courseid']}' ";
			$query .= "AND imas_fakeusers.id NOT IN ($curids) ORDER BY imas_fakeusers.LastName,imas_fakeusers.FirstName";
			$result = mysql_query($query) or die("Query failed : $query;  " . mysql_error());
			while ($row = mysql_fetch_row($result)) {
	...
	*/
	...
	if (isset($sessiondata['actas'])) {
		...
		$query = "SELECT LastName, FirstName FROM imas_fakeusers WHERE id='{$sessiondata['actas']}'";
		$result = mysql_query($query) or die("Query failed : $query:" . mysql_error());
		$row = mysql_fetch_row($result);
		echo $row[1].' '.$row[0];
		echo '<p>';
	...
	if (isset($_GET['action'])) {
		...
		if ($testsettings['isgroup']>0) {
			...
			if (!$isteacher || isset($sessiondata['actas'])) {
				...
				$query = "SELECT imas_fakeusers.id,imas_fakeusers.FirstName,imas_fakeusers.LastName FROM imas_fakeusers,imas_assessment_sessions WHERE ";
				$query .= "imas_fakeusers.id=imas_assessment_sessions.userid AND imas_assessment_sessions.agroupid='{$sessiondata['groupid']}' ORDER BY imas_fakeusers.LastName,imas_fakeusers.FirstName";
				$result = mysql_query($query) or die("Query failed : $query;  " . mysql_error());
				while ($row = mysql_fetch_row($result)) {
					$curgrp[] = $row[0];
	...
	function showscores($questions,$attempts,$testsettings) {
		...
		if ($isdiag) {
			global $userid;
			$query = "SELECT * from imas_fakeusers WHERE id='$userid'";
			$result = mysql_query($query) or die("Query failed : " . mysql_error());

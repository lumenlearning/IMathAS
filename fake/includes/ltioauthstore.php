...
class IMathASLTIOAuthDataStore extends OAuthDataStore {
	...
	if ($keyparts[0]=='cid' || $keyparts[0]=='placein') {
		...
		$query = "SELECT password,rights FROM imas_fakeusers WHERE SID='{$keyparts[1]}' AND (rights=11 OR rights=76)";
	} else {
		$query = "SELECT password,rights FROM imas_fakeusers WHERE SID='{$keyparts[0]}' AND (rights=11 OR rights=76)";
	}

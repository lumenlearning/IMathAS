...
if (isset($_POST['SID'])) {
	...
	if (!$noproctor) {
		if (!in_array(strtolower($_POST['passwd']),$basicpw) && !in_array(strtolower($_POST['passwd']),$superpw)) {
			...
			if ($passwordnotfound) {
				$query = "SELECT password FROM imas_fakeusers WHERE SID='$diagSID'";
				$result = mysql_query($query) or die("Query failed : " . mysql_error());
	...
	$query = "SELECT id FROM imas_fakeusers WHERE SID='$diagSID'";
	$result = mysql_query($query) or die("Query failed : " . mysql_error());
	if (mysql_num_rows($result)>0) {
	...
	$query = "INSERT INTO imas_fakeusers (SID, password, rights, FirstName, LastName, email, lastaccess) ";

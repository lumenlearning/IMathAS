...
//(c) 2009 David Lippman
	...
	//look up user
	$query = "SELECT id FROM imas_fakeusers WHERE remoteaccess='{$_GET['key']}'";
  ...
  function formatdate($date) {
	...
	if (count($lastpost)>0) {
		...
		$query = "SELECT imas_forum_posts.*,imas_fakeusers.LastName,imas_fakeusers.FirstName FROM imas_forum_posts,imas_fakeusers ";
		$query .= "WHERE imas_forum_posts.userid=imas_fakeusers.id AND imas_forum_posts.id IN ($threadids) ORDER BY imas_forum_posts.postdate DESC";
		$result = mysql_query($query) or die("Query failed : $query " . mysql_error());
		$lastforum = '';
		$lastcourse = '';
		$forumcontent = array();
	...
	$query = "SELECT imas_msgs.id,imas_msgs.courseid,imas_msgs.title,imas_msgs.senddate,imas_fakeusers.FirstName,imas_fakeusers.LastName ";
	$query .= "FROM imas_msgs,imas_fakeusers WHERE imas_msgs.msgto='$userid' AND imas_msgs.msgfrom=imas_fakeusers.id ";

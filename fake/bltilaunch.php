	...
	if ($_GET['userinfo']=='set') {	
		...
		if ($name_only) {
			...
			if (!empty($_POST['curSID']) && !empty($_POST['curPW'])) {
				...
				$query = "SELECT password,id FROM imas_fakeusers WHERE SID='{$_POST['curSID']}'";
				$result = mysql_query($query) or die("Query failed : " . mysql_error());
				if (mysql_num_rows($result)==0) {
		...
		if ($infoerr=='') { // no error, so create!
			...
			if (!isset($userid) && $allow_acctcreation) {	
				...
				if ($ltirole=='instructor') {
					...
					$query = "INSERT INTO imas_fakeusers (SID,password,rights,FirstName,LastName,email,msgnotify,groupid) VALUES ";
					$query .= "('{$_POST['SID']}','$md5pw',$rights,'{$_POST['firstname']}','{$_POST['lastname']}','{$_POST['email']}',$msgnot,$newgroupid)";
				} else {
					$rights = 10;
					$query = "INSERT INTO imas_fakeusers (SID,password,rights,FirstName,LastName,email,msgnotify) VALUES ";
...
if ($keyparts[0]=='cid' || $keyparts[0]=='placein') {
	...
	if (mysql_num_rows($result2)==0) {
		...
		$query = "SELECT imas_tutors.id FROM imas_tutors JOIN imas_fakeusers ON imas_tutors.userid=imas_fakeusers.id WHERE ";
		$query .= "imas_tutors.courseid='$cid' AND imas_fakeusers.SID='".addslashes($usid[0])."'";

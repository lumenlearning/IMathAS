<?php
	...
	if ($_GET['action']=="newuser") {
		...
		if (!isset($_GET['confirmed'])) {
			$query = "SELECT SID FROM imas_fakeusers WHERE email='{$_POST['email']}'";
			$result = mysql_query($query) or die("Query failed : " . mysql_error());
			if (mysql_num_rows($result)>0) {
				$nologo = true;
				require("header.php");
		...
		$query = "INSERT INTO imas_fakeusers (SID, password, rights, FirstName, LastName, email, msgnotify, homelayout) ";
		...
		require_once("config.php");
		$query = "UPDATE imas_fakeusers SET rights=10 WHERE id='{$_GET['id']}' AND rights=0";
		mysql_query($query) or die("Query failed : " . mysql_error());
		...
		if (isset($_POST['username'])) {
			$query = "SELECT password,id,email FROM imas_fakeusers WHERE SID='{$_POST['username']}'";
			$result = mysql_query($query) or die("Query failed : " . mysql_error());
			if (mysql_num_rows($result)>0) {
			...
			$query = "SELECT password FROM imas_fakeusers WHERE id='{$_GET['id']}'";
			$result = mysql_query($query) or die("Query failed : " . mysql_error());
			if (mysql_num_rows($result)>0 && $_GET['code']===mysql_result($result,0,0)) {
				$newpw = md5("password");
				$query = "UPDATE imas_fakeusers SET password='$newpw' WHERE id='{$_GET['id']}' LIMIT 1";
				mysql_query($query) or die("Query failed : " . mysql_error());
				echo "Password Reset.  ";
		...
		require_once("config.php");
		$query = "SELECT SID,lastaccess FROM imas_fakeusers WHERE email='{$_POST['email']}'";
		$result = mysql_query($query) or die("Query failed : " . mysql_error());
		if (mysql_num_rows($result)>0) {
	...
	if ($_GET['action']=="logout") {
		...
		$query = "UPDATE imas_fakeusers SET FirstName='{$_POST['firstname']}',LastName='{$_POST['lastname']}',email='{$_POST['email']}',msgnotify=$msgnot,qrightsdef=$qrightsdef,deflib='$deflib',usedeflib='$usedeflib',homelayout='$layoutstr',listperpage='$perpage'$chguserimg ";
		...
		if (isset($_GET['clear'])) {
			$query = "UPDATE imas_fakeusers SET remoteaccess='' WHERE id='$userid'";
			mysql_query($query) or die("Query failed : " . mysql_error());
		}
	}
	if ($isgb) {

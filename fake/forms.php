...
switch($_GET['action']) {
	...
	case "lookupusername":
		...
		break;
	case "googlegadget":
		$query = "SELECT remoteaccess FROM imas_fakeusers WHERE id='$userid'";
		$result = mysql_query($query) or die("Query failed : " . mysql_error());
		...
		if ($code=='' || isset($_GET['regen'])) {
			...
			do {
				...
				$query = "SELECT id FROM imas_fakeusers WHERE remoteaccess='$pass'";
				$result = mysql_query($query) or die("Query failed : " . mysql_error());
			} while (mysql_num_rows($result)>0);
			$query = "UPDATE imas_fakeusers SET remoteaccess='$pass' WHERE id='$userid'";

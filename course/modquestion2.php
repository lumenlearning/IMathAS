      ...
      $showwork = Sanitize::onlyInt($_POST['showwork']);
		...
		if (isset($_GET['id'])) { //already have id - updating
			if (isset($_POST['replacementid']) && $_POST['replacementid']!='' && intval($_POST['replacementid'])!=0) {
				...
				$settings = array(':points'=>$points, ':attempts'=>$attempts, ':penalty'=>$penalty, ':regen'=>$regen, ':showans'=>$showans, ':showwork'=>$showwork, ':rubric'=>$rubric,
				...
				$query .= " WHERE id=:id";
				$stm = $DBH->prepare($query);
				$settings = array(':points'=>$points, ':attempts'=>$attempts, ':penalty'=>$penalty, ':regen'=>$regen, ':showans'=>$showans, ':showwork'=>$showwork,

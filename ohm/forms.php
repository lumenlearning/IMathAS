...
switch($_GET['action']) {
      ...
      <input type=checkbox id=msgnot name=msgnot checked=checked aria-label=msgnot />
		...
		if (!$emailconfirmation) {
			...
			if ($stm->rowCount()>0) {
				$doselfenroll = true;
				echo "<br/>";
				echo '<p class=directions>Select the course you\'d like to enroll in</p>';
				echo '<select class="lumenform form" id="courseselect" name="courseselect" onchange="courseselectupdate(this);">';
				...
				echo "<br/>";
				echo '<p class=directions>If you already know your course ID, you can enter it now.</p>';
			}
			// <input class='lumenform form' type=\"text\" size=60 id=email name=email placeholder=Email  aria-label=email required>\n
	...
	case "googlegadget":
		...
		echo '<p>Add to iGoogle: <a href="http://fusion.google.com/add?source=atgs&moduleurl=http%3A//'.$GLOBALS['basesiteurl'].'/google-postreader.php"><img src="http://gmodules.com/ig/images/plus_google.gif" border="0" alt="Add to Google"></a></p>';

...
function disableInputs(qn, disabled) {
  ...
  for (var i = 0; i < disabled.length; i++) {
    ...
    if (pn === 'all') {
      ...
      regex = new RegExp(
        '^(qn|tc|qs)(' + qn + '\\b|' + ((qn + 1) * 1000 + pn * 1) + '\\b)'
      )
    } else {
        ...
        if ((m = el.name.match(regex)) !== null && !el.name.match(/-val/)) {
          ...
          if (el.type === 'radio' || el.type === 'checkbox') {
            if (el.checked && el.value !== el.getAttribute('data-initval')) {
              ...
              !el.checked &&
              el.value === el.getAttribute('data-initval')
            ) {
              thisChanged = true
            }
          } else {
            if (
              el.value.trim() !== el.getAttribute('data-initval') &&
              el.value.trim() !== ''
            ) {
              thisChanged = true
            }
          ...
          if ((el.type !== 'radio' && el.type !== 'checkbox') || el.checked) {
            if (el.type === 'file' && el.files.length > 0) {
              ...
              if (forbackground) {
                data.append(
                  el.name,
                  window.imathasAssess.preSubmitString(el.name, el.value)
                )
              } else {
                el.value = window.imathasAssess.preSubmitString(
                  el.name,
                  el.value
                )
              }

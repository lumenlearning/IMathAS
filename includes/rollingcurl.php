...
Class RollingCurlX {
    ...
    private $_postdatacallback = NULL; //callback for generating post data
    private $_timeout = 5000; //all individual requests must be completed by this time (in milliseconds)
    private $_stopAddingTime = 0; //stop adding requests to queue after this time (in seconds; 0: don't limit)
    ...
    public function addRequest(
        ...
        $this->requests[] = [
            ...
            'post_callback' => ($postdatacallback) ? $postdatacallback : $this->_postdatacallback,
    ...
    private function process_request($completed, $multi_handle, array &$requests_map) {
        ...
        if(curl_errno($ch) !== 0 || round(intval($request_info['http_code'])/100) != 2) {
          //if server responded with http error or a non 2xx code
            ...
            $request_info['response_text'] = $response;
            $response = false;

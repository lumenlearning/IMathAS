<div id="logo">
<img src="<?php echo $imasroot; ?>/wamap/img/wamaptxt.gif" alt="WAMAP.org: Washington Mathematics Assessment and Placement"/>
</div>

<ul id="navlist">
<li><a href="<?php echo $imasroot; ?>/index.php">About Us</a></li>
<li><a href="<?php echo $imasroot; ?>/wamap/info/classroom.php">Classroom</a></li>
<li><a href="<?php echo $imasroot; ?>/diag/index.php">Diagnostics</a></li>
<li><a href="<?php echo $imasroot; ?>/diag/index.php?id=12">MPT Practice</a></li>
<li><a href="<?php echo $imasroot; ?>/directaccess.php?cid=4647&getsid=true">Placement Review</a></li>
</ul>

<div id="header">
<img class="floatright" src="<?php echo $imasroot; ?>/img/graph.gif" alt="graph image" />
<div class="vcenter"><?php echo $pagetitle;?></div>
</div>

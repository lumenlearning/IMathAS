...
if ($showmessagesgadget) {
	...
	$query .= "imas_msgs.msgto=:msgto AND imas_msgs.viewed=0 ";
	...
	$stm = $DBH->prepare("SELECT courseid,COUNT(id) FROM imas_msgs WHERE msgto=:msgto AND viewed=0 GROUP BY courseid");
	$stm->execute(array(':msgto'=>$userid));

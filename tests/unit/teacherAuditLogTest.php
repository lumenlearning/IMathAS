...
final class TeacherAuditLogTest extends TestCase
{
    ...
    {
        $_SERVER['REQUEST_URI'] = 'teacherAuditLogTest.php';
        try {
     ...
     * Add array value
     */
    public function testAddInnerArrayValues()
    {
        $action = 'Change Grades';
        $item = self::$course1[0];
        $aarray = [':userid'=>1, ':score'=>10];
        $result = TeacherAuditLog::addTracking($item['courseid'], $action, $item['itemid'], ['inner array'=>$aarray]);
